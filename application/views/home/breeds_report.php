...
<div class="row mb-3">
    ...
    <thead style="background-color: #003366; color:#fff;">
        <tr>
            <th>Sl No</th>
            <th>Breed Name</th>
            <th style="display:none;">Updated Date</th> <!-- hidden -->
        </tr>
    </thead>
    <tbody>
        <?php foreach($all_breeds as $rkey => $rvals){ ?>
        <tr>
            <td><?= $rkey+1 ?></td>
            <td><?= $rvals['breeds_name'] ?></td>
            <td style="display:none;"><?= $rvals['updated_date'] ?></td> <!-- hidden -->
        </tr>
        <?php } ?>
    </tbody>
</table>
...
$(document).ready(function() {
    ...
    var table = $('#table_roles').DataTable({
        ...
        dom: 'Bfrtip',
        columnDefs: [
            { "targets": [2], "visible": false } // hide updated_date
        ],
        buttons: [
            { extend: 'excelHtml5', title: 'Breeds List' },
            { extend: 'pdfHtml5', title: 'Breeds List' },
