				...
				<div class="app-content">
					<div class="container">
						...
						<div class="page-header hor-pageheader">
							...
							<span class="mt-3 mt-md-0 pg-header">
										<a href="#" class="btn btn-info ml-0 ml-md-4 mt-1" data-toggle="modal" data-target="#modal-sample_type"><i class="typcn typcn-plus"></i> Add New Sample Type</a>
									</span>
						</div>
												...
												<tbody>
													...
													foreach($all_sample_type as $rkey => $rvals){ ?>
														<tr>
															...
															<td><a href="" class="btn_edit" data-id="<?php echo $rvals['sample_type_id']; ?>"><i class="fa fa-edit" type="button" title="Edit Sample Type"></i></a>
															<a href="#" class="btn_delete text-danger ml-2" data-id="<?php echo $rvals['sample_type_id']; ?>"><i class="fa fa-trash" title="Delete sample type"></i></a></td>
...
<script type="text/javascript">
	$(document).on('click', '#btn_save', function(e) {
		...
		var sample_type_name = $('#sample_type_name').val();
		//var description = $('#description').val();
		$.ajax({
			type: "POST",
			url: '<?= site_url('Sample_type/saveSample_type'); ?>',
			data: {
				edit_id:edit_id,
			...
			success: function(response) {
				if(response == 1){
					alert("Sample_type Saved");
					$('#sample_type_name').val('');
					//$('#description').val('');
					location.reload();
				}
				else if(response == 2){
					alert("This Sample Type alreay exist");
				}
				else if(response == 3){
					alert("This Sampletype Updated");
					$('#sample_type_name').val('');
					//$('#description').val('');
					location.reload();
				}
...
<script type="text/javascript">
	$(document).on('click', '.btn_edit', function(e) {
		...
		$.ajax({
			url: '<?= site_url('Sample_type/getsample_typeDatas'); ?>',
			method: 'POST',
			data: {
				sample_type_id: id
			},
			...
			success: function(response) {
				$('#edit_id').val(id);
				$('#sample_type_name').val(response[0].sample_type_name);
				$('#description').val(response[0].description);
				$('#btn_save').text("Update");
				$('#modal-sample_type').modal('show');
			},
...
$(document).on('click', '.btn_delete', function(e) {
    ...
    if (confirm("Are you sure you want to delete this sample type?")) {
        $.ajax({
            url: '<?= site_url('Sample_type/deleteSample_type'); ?>',
            type: 'POST',
            data: { sample_type_id: id },
            dataType: 'json',
            success: function(response) {
                if (response == 1) {
                    alert("Sample type deleted successfully!");
                    location.reload(); // refresh table or use DataTable reload if applied
                } else {
                    alert("Failed to delete sample type. Please try again.");
                }
            },
            error: function(xhr, status, error) {
